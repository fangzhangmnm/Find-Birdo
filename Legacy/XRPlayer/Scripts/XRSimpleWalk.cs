...
public class XRSimpleWalk : MonoBehaviour
{
    ...
    void MoveCharacter(Vector3 moveVelocityWS, out bool isThisStepHit)
    {
        ...
        if (moveVelocityWS.magnitude > 0 && isNextStepHit && Vector3.Dot(nextStepHit.normal, GravityUp) > Mathf.Cos(Mathf.Deg2Rad * slopeLimit))
        {
            ...
            Vector3 targetVelocity= (nextStep - transform.position).normalized * moveVelocityWS.magnitude;
            /* //No! acceleration will introduce motion sickness
            Vector3 forceNeeded = (targetVelocity - body.velocity) / Time.fixedDeltaTime * body.mass;
            body.AddForce(Vector3.ClampMagnitude(forceNeeded, walkFriction * body.mass * Physics.gravity.magnitude));
            */
