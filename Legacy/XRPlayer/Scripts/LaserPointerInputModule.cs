...
namespace Wacki {
    ...
    public class LaserPointerInputModule : BaseInputModule {
        ...
        {
            raycaster.eventMask = eventMask;

            foreach(var pair in _controllerData)if(pair.Key.isActiveAndEnabled)
            {
                ...
                if (data.pointerEvent.pointerCurrentRaycast.distance > 0.0f)
                    controller.SetLaserDistance(data.pointerEvent.pointerCurrentRaycast.distance + 0.01f);
                else
                    controller.SetLaserDistance(0);
                ...
                if(controller.ButtonDown()) {
                    if (!controller._hasButtonDown)
                    {
                        ...
                        controller._hasButtonDown = true;
                    }
                }// button down end
                else
                {
                    controller._hasButtonDown = false;
                }
